    ...
    if (userInput.trim() !== "") {
      ...
      try {
        ...
        const response = await fetch(`${AGENT_BACKEND_URL}/chat`, {
          ...
          body: JSON.stringify({
            message: userInput,
            recentMessages: currentSession
              ? Object.entries(currentSession.requests).flatMap(
                  ([key, value]) => {
                    const userMessage = key.substring(0, key.lastIndexOf("_"));
                    const botMessage = value.result;

                    return [{ user: userMessage }, { agent: botMessage }];
                  }
                )
              : [],
          }),
