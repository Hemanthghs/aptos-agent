...
// Add the interface for message generation
...
declare function generateMessageResponse(params: GenerateMessageParams): Promise<string>;
declare function parseResponse(response: string): { text: string } | null;
...
export class BaseAgentRuntime implements AgentRuntime {
    ...
    async generateResponse(userText: string): Promise<string> {
        try {
            ...
            const messageHandlerTemplate = this.createMessageTemplate(userText, summary);
