    ...
    try {
        ...
        setTimeout(async () => {
            ...
            for (let index = 0; index < urls.length; index++) {
                ...
                console.info(`Processing knowledge source ${index + 1}/${urls.length}: ${url}`);
...
app.post("/chat", async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.info("Processing chat message:", chatRequest.message.substring(0, 50) + (chatRequest.message.length > 50 ? "..." : ""));
        let responseText = ''
        if (runtime) {
